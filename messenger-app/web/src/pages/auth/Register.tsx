...
export default function Register() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setErr(null);
    try {
      const t = await register({ email, username, password });
  ...
  return (
    <div className="min-h-dvh flex items-center justify-center p-4">
      <form onSubmit={onSubmit} className="w-full max-w-sm space-y-3 border rounded-lg p-4">
        ...
        <button className="w-full bg-blue-600 text-white rounded p-2" type="submit">Create account</button>
        <div className="text-sm text-center">
          Have an account? <Link className="underline" to="/auth/login">Login</Link>
        </div>
      </form>
    </div>
  );
